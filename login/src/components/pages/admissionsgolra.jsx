...
const AdmissionsGolra = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const carResponse = await axios.get(`http://localhost:5000/api/cars/${branch}`);
        setCars(carResponse.data);
        const instructorResponse = await axios.get(`http://localhost:5000/api/instructors/${branch}`);
  ...
  const handleCourseChange = (e) => {
    ...
    const selectedCourseDetails = courseData[category].find(course => course.days === days.trim());

    if (selectedCourseDetails) {
      const durationInMinutes = selectedCourseDetails.time.includes('2 Hours') ? 120 : parseInt(selectedCourseDetails.time);
      setAvailableEvents(generateAvailableEvents(durationInMinutes));
      setFormData(prev => ({ ...prev, courseDuration: parseInt(days.match(/\d+/)[0]) }));
    }
  };
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    if (!navigator.onLine) {
      setFormError('No internet connection. Data saved locally and will submit automatically once online.');
      return;
    }
  
    if (!selectedVehicle || !selectedCourse || !formData.lecturerCode || !formData.dateRegistered || !recurringEvents.length) {
      setFormError('Please complete all fields including selecting a time slot on the calendar');
      return;
    }
    ...
    try {
      const response = await axios.post(`http://localhost:5000/api/admissions/${branch}/submit`, admissionData);
  ...
  }, [selectedVehicle, selectedCourse, formData, recurringEvents, paymentDetails, selectedInstructor, remainingAmount]);
  ...
  return (
      ...
      <div className="admission-form">
        ...
        {referenceNumber && <p className="success-message">Generated Reference Number: {referenceNumber}</p>}
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            <div className="input-group">
              ...
              <select value={formData.lecturerCode} onChange={handleInstructorChange} required>
          ...
          <label>
            CHECK <Link to="/sidebargolra/schedulegolra" style={{ color: '#007bff' }}>SCHEDULE PAGE</Link> IF TIME SLOT IS AVAILABLE FOR THIS INSTRUCTOR
          </label>
          <br/>
            ...
            <div className="modal-content">
              <button
                ...
                onClick={() => {
                  ...
                  window.location.href = '/sidebargolra/schedulegolra';
                }}
              >
                ×
              </button>
...
export default AdmissionsGolra;
