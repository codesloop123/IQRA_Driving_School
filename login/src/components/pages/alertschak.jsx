...
const AlertsChak = () => {
  const branch = 'chaklala'; // Specify the branch name here
  ...
  useEffect(() => {
    ...
    const fetchAlerts = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/alerts/payments/${branch}`);
        setAlerts(response.data);
        localStorage.setItem(`alerts_${branch}`, JSON.stringify(response.data)); // Save to local storage
      } catch (err) {
        setError('Error fetching payment alerts. Loading data from local storage.');
        console.error(err);
  ...
  const handleCloseModal = async (alertId) => {
    ...
    if (navigator.onLine) {
      try {
        ...
        await axios.post(`http://localhost:5000/api/alerts/complete/${alertId}`, {
          newAmountReceived: newAmountReceived,
        });
        // Remove the alert from displayed list if the balance is cleared
        setAlerts(alerts.filter(alert => alert._id !== alertId));
        localStorage.setItem(`alerts_${branch}`, JSON.stringify(alerts.filter(alert => alert._id !== alertId)));
      } catch (err) {
        console.error('Error completing the payment:', err);
        alert('Error completing payment. The update has been saved locally and will sync when online.');
      }
    } else {
      // If offline, mark alert for later sync
      const unsyncedAlerts = JSON.parse(localStorage.getItem(`unsynced_alerts_${branch}`)) || [];
      unsyncedAlerts.push({ alertId, newAmountReceived });
      localStorage.setItem(`unsynced_alerts_${branch}`, JSON.stringify(unsyncedAlerts));
      alert('No internet connection. Payment completion will be synced when online.');
    }
  };
  ...
  useEffect(() => {
    const syncUnsyncedData = async () => {
      const unsyncedData = JSON.parse(localStorage.getItem(`unsynced_alerts_${branch}`)) || [];
      if (unsyncedData.length > 0) {
        for (const { alertId, newAmountReceived } of unsyncedData) {
          try {
            await axios.post(`http://localhost:5000/api/alerts/complete/${alertId}`, { newAmountReceived });
        ...
        alert('Unsynced payment completions have been successfully synced with the server.');
      }
    };
  ...
  return (
    <div className="alerts-container">
      <h2>Payment Alerts for {branch.charAt(0).toUpperCase() + branch.slice(1)} Branch</h2>
          ...
          <tbody>
            {alerts.map(alert => (
              <tr key={alert._id}>
                ...
                <td>{alert.firstName} {alert.fatherName}</td>
      ...
      {showModal && invoiceData && (
        <InvoiceModal
          data={invoiceData}
          onClose={() => handleCloseModal(alerts.find(alert => alert.referenceNumber === invoiceData.referenceNumber)._id)}
        />
      )}
    </div>
  );
